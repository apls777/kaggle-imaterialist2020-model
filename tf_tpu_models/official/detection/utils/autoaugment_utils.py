...
def cutout(image, pad_size, replace=0):
    ...
    replace = tf.concat(
        [
            tf.cast(replace, dtype=image.dtype),
            tf.zeros([num_channels - 3], dtype=image.dtype),
        ],
        axis=0,
    )
...
def random_shift_bbox(image, bbox, pixel_scaling, replace, new_min_bbox_coords=None):
    ...
    # with gray (128).
    bbox_content = image[
        shifted_min_y : shifted_max_y + 1, shifted_min_x : shifted_max_x + 1, :
    ]
...
def _apply_bbox_augmentation_wrapper(
    image, bbox, bbox_idx, new_bboxes, prob, augmentation_func, func_changes_bbox, *args
):
    ...
    if func_changes_bbox:
        ...
        augmented_image = tf.cond(
            should_apply_op,
            lambda: _apply_bbox_augmentation(
                image, bbox, bbox_idx, augmentation_func, *args
            ),
            lambda: image,
        )
...
def _apply_multi_bbox_augmentation(
    ...
    # will be thrown away.
    bboxes = tf.cond(
        tf.equal(tf.size(bboxes), 0), lambda: tf.constant(_INVALID_BOX), lambda: bboxes
    )
    ...
    wrapped_aug_func = (
        lambda _image, bbox, bbox_idx, _new_bboxes: _apply_bbox_augmentation_wrapper(
            _image,
            bbox,
            bbox_idx,
            _new_bboxes,
    ...
    body = lambda _idx, _images_and_bboxes: [
        _idx + 1,
        wrapped_aug_func(
            _images_and_bboxes[0], loop_bboxes[_idx], idx, _images_and_bboxes[1]
        ),
    ]
    # pylint:enable=g-long-lambda
...
def _apply_multi_bbox_augmentation_wrapper(
    image, bboxes, prob, aug_func, func_changes_bbox, *args
):
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
    ...
    rotation_matrix = tf.stack(
        [[tf.cos(radians), tf.sin(radians)], [-tf.sin(radians), tf.cos(radians)]]
    )
    new_coords = tf.cast(
...
def _extract_bboxes_from_masks(image):
    ...
    bbox_ymin = (
        tf.reduce_min(height_proj_indices + image_height * (1.0 - height_proj), axis=0)
        / image_height
    )
    bbox_xmin = (
        tf.reduce_min(width_proj_indices + image_width * (1.0 - width_proj), axis=0)
        / image_width
    )
...
def _cutout_inside_bbox(image, bbox, pad_fraction):
    ...
    mean = tf.reduce_mean(
        image[min_y : max_y + 1, min_x : max_x + 1], reduction_indices=[0, 1]
    )
...
def bbox_cutout(image, bboxes, pad_fraction, replace_with_mean):
    ...
    def apply_bbox_cutout(image, bboxes, pad_fraction):
        ...
        replace = tf.concat(
            [
                tf.ones(3, dtype=image.dtype) * tf.cast(replace, image.dtype),
                tf.zeros([tf.shape(image)[2] - 3], dtype=image.dtype),
            ],
            axis=0,
        )
...
def _apply_func_with_prob(func, image, args, prob, bboxes):
    ...
    augmented_image, augmented_bboxes = tf.cond(
        should_apply_op, lambda: func(image, bboxes, *args), lambda: (image, bboxes)
    )
